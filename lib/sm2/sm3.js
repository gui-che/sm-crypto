...
var SM3Digest = function () {
    function SM3Digest() {
        (0, _classCallCheck3['default'])(this, SM3Digest);
    ...
    (0, _createClass3['default'])(SM3Digest, [{
        key: 'init',
        value: function init() {
        ...
        value: function urShiftLong(number, bits) {
            var returnV = void 0;
            var big = new _jsbn.BigInteger();
            big.fromInt(number);
            if (big.signum() >= 0) {
                returnV = big.shiftRight(bits).intValue();
            } else {
                var bigAdd = new _jsbn.BigInteger();
                bigAdd.fromInt(2);
                ...
                if (shiftLeftBits < 0) {
                    ...
                    var shiftLeftNumberBigAdd = new _jsbn.BigInteger();
                    shiftLeftNumberBigAdd.fromInt(number >> bits);
                    var shiftLeftNumberBig = new _jsbn.BigInteger("10" + shiftLeftNumber, 2);
        ...
        value: function getZ(g, publicKey) {
            var userId = _utils2['default'].parseUtf8StringToHex('1234567812345678');
            var len = userId.length * 4;
            this.update(len >> 8 & 0x00ff);
            this.update(len & 0x00ff);
            var userIdWords = _utils2['default'].hexToArray(userId);
            this.blockUpdate(userIdWords, 0, userIdWords.length);
            var aWords = _utils2['default'].hexToArray(g.curve.a.toBigInteger().toRadix(16));
            var bWords = _utils2['default'].hexToArray(g.curve.b.toBigInteger().toRadix(16));
            var gxWords = _utils2['default'].hexToArray(g.getX().toBigInteger().toRadix(16));
            var gyWords = _utils2['default'].hexToArray(g.getY().toBigInteger().toRadix(16));
            var pxWords = _utils2['default'].hexToArray(publicKey.substr(0, 64));
            var pyWords = _utils2['default'].hexToArray(publicKey.substr(64, 64));
